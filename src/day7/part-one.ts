...
/////////////////////// Utils ///////////////////////////
function readFile() {
    let file = fs.readFileSync("./src/day7/input.txt", {encoding: "utf-8"})
 ...
 * @param hand
 */
export function sortCardsByLabel(hand: HandOfCards) {
    return hand.toSorted((a, b) => getLabelValue(a) > getLabelValue(b) ? -1 : 1)
}


console.log(sortCardsByLabel("HJ232".split("") as HandOfCards))
 ...
 * @link HandRank
 */
export function evaluateHandRanking(match?: string) {
    switch (match?.length) {
        case 5:
...
function isAFullOFFive(card:Label[]){
  return card.length === 5
}
...
export function parseHand(hand: HandOfCards): number {
    const sortedHand = sortCardsByLabel(hand)
    ...
    let lengthOfFirst = evaluateHandRanking(first)
    let lengthOfSecond = evaluateHandRanking(second)


    switch (true) {
        case isAFullHouse(lengthOfFirst, lengthOfSecond):
            return HandRank.fullHouse;
        case isADoublePair(lengthOfFirst,lengthOfSecond):
            return HandRank.doublePair;
    }
    return (lengthOfFirst || lengthOfSecond) || 0

}

////////// matchCardPattern ////////////
function isAFullHouse(firstCombinaison ?: number , secondCombinaison?: number) {
  return (firstCombinaison == HandRank.pair && secondCombinaison === HandRank.brelan ) || (secondCombinaison === HandRank.pair && firstCombinaison === HandRank.brelan)
}

function isADoublePair(firstCombinaison ?: number , secondCombinaison?: number) {
  return firstCombinaison === HandRank.pair && secondCombinaison == HandRank.pair;
...
function run() {
    ...
    const setsOfCards: [HandOfCards, number][] = lines.map((ln) => ln.split(" ")).map(([data, bid]) => [data.split("") as HandOfCards, parseInt(bid)])

    // Sort the the sets of Hand
    const sortedSetsByRank: [HandOfCards, number][] = setsOfCards .toSorted((
            current,
            next
        ) => {
